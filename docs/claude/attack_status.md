...
- 제안 (미합의): 이동 비용 가중치 = 몬스터 1마리당 5칸 (Dodge.Step 근거, 실측 조정 대상)
- 제안 (미합의): 이동 문턱 = 가중치 1마리분 이상 개선 시에만 이동. 현행 `c.mc >= baseline → continue`(638)와 같은 의미
- 제안 (미합의): 루프의 Angle/Detour 게이트 제거
