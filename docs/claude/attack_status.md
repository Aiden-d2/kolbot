...
- 제안 (미합의): 모드별 처리. clear는 2·3단에서 버림. clearList는 2단은 보류, 3단은 대응 단계 상승(flash → 다른 각도 → 보조 스킬 → 이동 제한 완화 → 명시적 실패). 정체 판정은 시간 기준
- 제안 (미합의): 엔진 호출이 `targets`(필수 대상, gid 추적)와 `sweep`(주변 소탕, 고정 기준점 range 또는 박스)을 동시에 받는 구조. 보스는 목줄(leash) 규칙으로 평소 거리순, 이탈이나 도주 시 우선. 시야를 잃으면 마지막 위치로 이동해 재탐색
...
- 미결: clearList 대상에 유효 스킬이 없을 때 처리, 목줄 길이 L과 위험 반경 값, 주로 놓치는 보스와 도주 패턴
