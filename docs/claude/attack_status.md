...
## 5-0. 구현 상태 (260926)
1차 구현 완료, 인게임 미검증. 구현 세부와 검증 상태는 `attack_design.md` 9절. 3절의 결함 중 3-1, 3-1b, 3-2, 3-3, 3-4(retry 공유), 3-5, 3-6은 새 엔진에서 구조적으로 해소 대상이다 (실측 확인 전).
...
- 진단 (제안): 근본 원인은 모드마다 선택 기준이 다르다는 것. 접근은 first-match(위협·이동 비용 미평가), 회피는 위협 점수인데 접근 중에는 미실행
- 제안 (미합의): 단일 목적함수
  - 타깃 사거리와 LOS를 만족하는 후보 중 `위협 + 이동 비용` 최소
  - 후보 반경 2~3개
  - 점수순 정렬 후 비싼 검사를 순서대로 해서 first-valid
  - 현 위치가 유효하면 이동 문턱(히스테리시스) 이상 개선될 때만 이동
- **[사실/합의] getPath 비용**: 루프에 0x1 게이트를 추가하면 getPath가 폭증한다. 루프의 0x4 Angle → Detour는 현행 유지(사용자 근거: getPath가 가장 비싸다)
- 제안 (미합의): 0x1 우회는 setPosition에서 직선 걷기 후보가 없어 `moveTo`로 가기 직전에만 판정
  - moveTo가 어차피 getPath를 부르므로 추가 getPath는 0~1회
  - 비율(DetourPath) 초과 시 "도달 불가" 기록 → 루프가 gid를 2단 캐시에 넣어 재계산하지 않음
- **[사실] 틱 내 진동은 현 구성에서 없다**
  - Necro 저주(137, 150)와 Sorc Static(66)은 호출부가 사거리 밖일 때만 setPosition을 부른다 → 접근 전용, 단조 이동
  - 가드 없는 이중 호출은 Necro timed(239~271) + untimed(294)뿐인데, N.SUMM은 untimed -1이라 미발생. "틱당 1회" 규칙은 불채택, 잠재 사항으로만 기록
- **[확정] 후보 반경**: 사거리, 사거리×0.75 두 개. 사거리 3 이하 근접 스킬은 1개
- **[확정] 도달 한계**: 경로 길이 / 직선거리 > `DetourPath`(4)면 도달 불가
- **[확정] 위치 선택 통일** (사용자: 실이 없으면 채택). 알려진 실:
  - 접근 시 위협 계산 비용이 소폭 증가 (거리 연산만, 몬스터 목록은 루프 스캔 재사용)
  - 접근 시 측면 후보를 선택할 수 있음. 경로상 몬스터는 미평가
  - Dodge 비활성 빌드는 이동 비용만 봄 (현행 first-match와 유사)
- **[확정] 이동 비용 가중치 없음** (사용자 의문 수용: 후보가 같은 반경 링 위에 있어 거리 차이의 실익이 작다)
  - 접근: ±90° 이내 후보 중 위협 최소, 동점이면 정면 순 (Dodge 비활성이면 위협 미평가, 현행 first-match와 동일)
  - 회피: 현 위치 대비 1마리 이상 감소하는 후보 중 최소, 동점이면 정면 순 (현행과 동일)
- **[확정] 이동 문턱 = 1마리** (현행 `c.mc >= baseline → continue`(638) 유지)
- **[확정] 루프의 Angle/Detour 게이트 제거**
  - 알려진 작은 차이: setPosition은 `CollMap.checkColl`(방 정보 없으면 막힘 5)을 쓰고, 게이트는 내장 `checkCollision`을 쓴다. 가시 범위 안 대상이라 발생 가능성은 낮다
  - 근거: 게이트가 필요했던 이유는 setPosition 실패 사유를 루프가 몰라 재시도 5회 + flash를 거쳤기 때문. 사이드채널로 "도달 불가"를 알면 첫 실패에서 즉시 버리고 gid 캐시 → 목록 축소 효과 동일
  - 추가 이점: 실제 시전 스킬 사거리로 판정(C-7 근사 문제 해소), 매 틱 반복 계산 제거
  - 조건: clear는 재시도 없이 즉시 버림, clearList는 대응 단계 상승. 306 Skip은 특수 예외로 유지. NoSkipArea에서는 버리지 않음
...
- **[확정] 위험 반경 10**: 보스 추적 중에도 먼저 처리 (Dodge.Range 13보다 작게)
- **[확정] 도주 대응은 단순 규칙**: 보스와의 거리가 25 이상이면 보스 우선. 별도 도주 감지 로직은 넣지 않는다. 층위: 10 위험 / 25 보스 우선 / 35 목줄
- **[확정] 틱 구조**: 현행 순서 유지. 스캔·범위 판정 → 대상 선정(목줄) → 위치 → 시전 → 결과 기록
- 사실: 사용자 인식은 "유효 스킬이 없는 빌드는 없다"(팀 기준). 봇 단위로는 S.FIRE 1~49(`[5]/[6] = -1`, `builds/S.FIRE.js:137`)가 화염 면역에, S.COLD 1~23이 냉기 면역에 결과 2
- **[확정] 유효 스킬이 없는 필수 대상은 전용 로직 없이 최소 처리.** 정체 판정(시간 기준)에 걸리면 명시적 실패를 반환하는 일반 경로로 흘려보낸다
  - 근거(사용자): S.FIRE는 50레벨부터 보조 스킬이 설정되고, 면역 몹은 나이트메어 후반부터 등장해 대부분 그 이후에 만난다. 운영상 문제되지 않음
  - 단, 버린 gid 재진입 방지(3-1)는 결과 0 경로에도 필요하므로 그대로 반영
  - 사실(사용자): Telekinesis(43)는 면역과 관계없이 통하는 스킬
  - 코드상 `getSkillElement`에 43 예외가 없어 `skills.txt` etype에 의존한다. etype이 특정 속성이면 checkResist가 해당 면역 몹에서 TK를 폴백에서 제외할 수 있다(저장소에 skills.txt 없음, 미확인)
  - 제안: 43을 243/249와 같이 `"none"` 예외에 추가. 1차 구현에 포함
- **[확정] 반환 계약은 0/1/2 유지. 사이드채널로 보완**
  - 루프가 doAttack 직전에 기록을 초기화
  - `Skill.cast`(`Misc.js:247`, 직업 파일 공격의 유일한 경로)와 `setPosition`이 시전 여부와 이동 결과를 Attack 상태에 기록
  - 루프가 반환값과 기록을 함께 읽는다. 직업 파일은 수정하지 않는다
  - 2차 대응(각도, 보조 스킬)도 Attack 상태의 "틱 지시사항"으로 전달하는 방향 검토
