...
//---------------------------------------be-sure-of-what-you-edit-under-this-line--------------------------------------------//
		Leader = false, // SiC-666 TODO: Change this to Leader (should be able to teleport).
		Boer = false, // SiC-666 TODO: Change this name?
...
//SYNCING
	...
	this.start = function () {
		...
		Pather.moveTo(me.x + (me.gid & 1 ? 5 : -5), me.y + (me.gid & 2 ? 5 : -5)); // Move off of waypoint so others can reach it.	//260822
		...
		while (!teamReady) {
			delay(500);
			if (getTickCount() - tick > 3 * 60 * 1000) { // Quit after 3 minutes of waiting.
				//D2Bot.restart();
				quit();
			}
		}
	...
	this.teamInGame = function (stayInGame) { // Counts all the players in game. If the number of players is below Team either return false or quit game depending on input.
		var i, player,
			count = 0;
		...
		if (count < Team.Size) {
			return stayInGame ? false :
			//D2Bot.restart();
			quit(); // Return false if stayInGame is true, otherwise leave game.
		}
	...
	this.syncBO = function (act) { //260813
		...
		while (!BOReady) {
			...
			if (getTickCount() - tick > 30 * 1000) { // Quit after 2 minutes of waiting.
				//D2Bot.restart();
				quit();
			}
		}
		...
		if (destination === 2) {
			...
			delay(me.ping * 2 + 200);
			Pather.moveTo(me.x + (me.gid & 1 ? 5 : -5), me.y + (me.gid & 2 ? 5 : -5));	//260822
		}
...
//PATHING
	...
	this.travel = function (goal) { // 0->9, a custom waypoint getter function
		...
		case 6:
			...
			//wpAreas = [75, 76, 77, 78, 79, 80, 81, 83];	//260728
...
//QUESTING
	...
	this.cain = function () { // Dark-f: rewrite rescue cain
		...
		if (!me.getQuest(4, 1)) { // not rescues Cain
			...
			Pather.moveToPreset(me.area, 1, 737, (me.gid & 1 ? 5 : -5), (me.gid & 2 ? 5 : -5), true, true);	//260822
	...
	this.trist = function () {
		...
		Pather.moveToPreset(me.area, 1, 737, (me.gid & 1 ? 5 : -5), (me.gid & 2 ? 5 : -5), true, true); //260719	//260822
		...
		for (i = 0; i < path.length; i += 2) {	//260627
			Pather.moveTo(path[i] + (me.gid & 1 ? 5 : -5), path[i + 1] + (me.gid & 2 ? 5 : -5), 10, true);	//260822
			//Attack.clear(25);
		}
	...
	this.smith = function () {	//260531
		...
		if (smith) {
			...
			while (getDistance(me.x, me.y, smith.roomx * 5 + smith.x, smith.roomy * 5 + smith.y) > 15) {
				...
				Pather.moveTo(smith.roomx * 5 + smith.x + (me.gid & 1 ? 5 : -5), smith.roomy * 5 + smith.y + (me.gid & 2 ? 5 : -5), 3, true);	//260822
			}
	...
	this.andy = function () {
		...
		while (!this.partyLevel(teleLvl)) {	//260530
		...
		for (i = 66; i <= 72; i += 1) {
			...
			if (chest) {
				while (getDistance(me.x, me.y, chest.roomx * 5 + chest.x, chest.roomy * 5 + chest.y) > 25) {
					try {
						Pather.moveTo(chest.roomx * 5 + chest.x + (me.gid & 1 ? 5 : -5), chest.roomy * 5 + chest.y + (me.gid & 2 ? 5 : -5), 3, true);	//260822
					} catch (e) {
						print("Caught Error.");
						print(e);
					}
...
*/
		...
		while (!me.getQuest(20, 0) && leaderFigurine === false) {
			delay((me.gid & 7) * 5000 + 1);
		}
		
		if (!me.getQuest(20, 0) && againFigurine === true) {
			delay((me.gid & 1) * 5000 + 1);
		}
	...
	this.travincal = function () {
		...
		//Pather.moveTo(5148 + (me.gid & 1 ? 5 : -5), 5066 + (me.gid & 2 ? 5 : -5), 5);	//260822
		...
		Pather.moveTo(presetUnit.roomx * 5 + presetUnit.x + 109 + (me.gid & 1 ? 5 : -5), presetUnit.roomy * 5 + presetUnit.y - 95 + (me.gid & 2 ? 5 : -5));	//260822
	...
	this.mephisto = function (takeRedPortal) {
		...
		if (!takeRedPortal) {
			...
			Pather.moveTo(17515 + (me.gid & 1 ? 5 : -5), 8061 + (me.gid & 2 ? 5 : -5), 3, true);	//260822
			Attack.clear(35);
	...
	this.izual = function () {
		...
		if (!me.getQuest(25, 1)) {
			...
			while (!me.getQuest(25, 1)) {
				Attack.clearList(Attack.scanList(256), null, 1);	// Izual
				//Attack.kill(256); // Izual
				sendPacket(1, 0x40);
				
				if (getTickCount() - fail > 60 * 1000) {
	...
	this.diablo = function () {
		...
		this.followPath = function (path) {
			...
			for (i = 0; i < path.length; i += 2) {
				Pather.moveTo(path[i] + (me.gid & 1 ? 5 : -5), path[i + 1] + (me.gid & 2 ? 5 : -5), 3, true);	//260822
			}
		};
		...
		Pather.moveTo(7790 + (me.gid & 1 ? 5 : -5), 5290 + (me.gid & 2 ? 5 : -5), 5, true);	//260822
	...
	this.ancients = function () { // SiC-666 TODO: Rewrite this.
		...
		Pather.moveTo(10048 + (me.gid & 1 ? 5 : -5), 12634 + (me.gid & 2 ? 5 : -5));	//260822
		...
		this.preattack = function () {
			...
			case 1: // Sorceress
				...
				case 64:	// Frozen Orb
					if (me.getState(121)) {
						...
						return Skill.cast(Config.AttackSkill[1], 0, 15093 + rand(-5, 5), 5028);	//skillcast
					}
			...
			case 5: // Druid
				if (Config.AttackSkill[3] === 245) {
					return Skill.cast(Config.AttackSkill[3], 0, 15093 + rand(-5, 5), 5028);	//skillcast
				}
			...
			case 6: // Assassin
				if (Config.UseTraps) {
					...
					baalTrap = ClassAttack.checkTraps({x: 15094, y: 5028});

					if (baalTrap) {
						ClassAttack.placeTraps({x: 15086, y: 5036}, 1);
						delay(150);
						ClassAttack.placeTraps({x: 15102, y: 5036}, 1);
						delay(150);
						ClassAttack.placeTraps({x: 15086, y: 5020}, 1);
						delay(150);
						ClassAttack.placeTraps({x: 15102, y: 5020}, 1);
						delay(150);
						ClassAttack.placeTraps({x: 15094, y: 5040}, 1);
					}
				}
	...
	BaalLoop:	//260629
		while (true) {
			...
			if (wave) {
				...
				if (me.classid === 3) {	//pal
					Pather.moveTo(15092 + (me.gid & 1 ? 0 : -5), 5030);	//260830
...
//FARMING
	this.farmingCountess = function() { 
		...
		Pather.moveToPreset(me.area, 2, 580, (me.gid & 1 ? 5 : -5), (me.gid & 2 ? 5 : -5), true, true);	//260822
	...
	this.farmingAndy = function() { 
		...
		Pather.moveTo(22549 + (me.gid & 1 ? 5 : -5), 9520 + (me.gid & 2 ? 5 : -5));	//260822
	...
	this.farmingCows = function () {	//260628
		...
		var getLeaderUnit = function (leaderName) {
			var getLeader;
			
			if (!leaderName) {
				print("leaderName undefined");
				return false;
			}
			
			getLeader = getUnit(0, leaderName);
			
			if (getLeader && copyUnit(getLeader).x) {
				//print("getUnit: " + getLeader.x + ", " + getLeader.y + "type: " + typeof getLeader.x + "mode: " + getLeader.mode);
				return getLeader;
			}
		
			//print("getLeaderUnit failed");
			
			return false;
		};

		var followLeader = function () {	//260903
			var getLeader, result,
				leaderName = getLeaderName();
			
			me.overhead("followLeader start");
			...
			while (!msgLeader) {
				...
				getLeader = getLeaderUnit(leaderName);
				
				if (getLeader) {
					me.overhead("Leader: " + getLeader.x + "." + getLeader.y + " distance: " + Math.round(getDistance(me, getLeader)));
					//print("Leader: " + getLeader.x + "." + getLeader.y + " distance: " + Math.round(getDistance(me, getLeader)));
					
					if (getDistance(me, getLeader) > 15) {
						Pather.moveTo(getLeader.x + (me.gid & 1 ? 5 : -5), getLeader.y + (me.gid & 2 ? 5 : -5));
						...
						//result = Pather.moveTo(getLeader.x + (me.gid & 1 ? 5 : -5), getLeader.y + (me.gid & 2 ? 5 : -5), 3, true);
					...
					if (msgNode) {
						me.overhead("node: " + msgNode[0] + "." + msgNode[1] + " distance: " + Math.round(getDistance(me, msgNode[0], msgNode[1])));
						...
						if (getDistance(me, msgNode[0], msgNode[1]) > 15) {
							Pather.moveTo(msgNode[0] + (me.gid & 1 ? 5 : -5), msgNode[1] + (me.gid & 2 ? 5 : -5));
							...
							//result = Pather.moveTo(msgNode[0] + (me.gid & 1 ? 5 : -5), msgNode[1] + (me.gid & 2 ? 5 : -5), 3, true);
		...
		//Pather.moveTo(me.x + (me.gid & 1 ? 5 : -5), me.y + (me.gid & 2 ? 5 : -5), 1, true);
		...
		if (Leader) {
			...
			if (!followLeader()) {
				Messaging.sendToList(Team.Profiles, "earlyReturn");
				print("earlyReturn: followLeader failed");
	...
	this.farmingSummoner = function() {
		...
		if (Leader) {
			...
			while (me.inTown) {
				...
				if (i % 20 == 0) { // Check for Team Members every 5 seconds.
					this.teamInGame();
				}
	...
	this.farmingMephisto = function() {
		...
		Pather.moveTo(17566 + (me.gid & 1 ? 5 : -5), 8069 + (me.gid & 2 ? 5 : -5)); // reportal bridge	//260822
	...
	this.farmingAbaddon = function() {
		...
		while (getDistance(me.x, me.y, presetUnit.roomx * 5 + presetUnit.x, presetUnit.roomy * 5 + presetUnit.y) > 15) {
			...
			Pather.moveTo(presetUnit.roomx * 5 + presetUnit.x + (me.gid & 1 ? 5 : -5), presetUnit.roomy * 5 + presetUnit.y + (me.gid & 2 ? 5 : -5), 3, true);	//260822
			delay(me.ping * 2 + 200);
		}
	...
	this.farmingNihlathak = function() { 
		...
		Pather.moveToPreset(me.area, 2, 462, (me.gid & 1 ? 5 : -5), (me.gid & 2 ? 5 : -5), true);	//260822
...
//MAIN
	...
	while (!this.teamInGame(true)) { // Wait for the AutoSmurf Team to join.
		...
		if (getTickCount() - tick > 2 * 60 * 1000) { // Leave the game after 2 minutes of waiting.
			//print("Team wasn't in game within 2 minutes.");
	...
	if (!me.getQuest(23, 0) && me.getQuest(15, 0)) { // "Able to go to Act IV" (AKA haven't gone thru red portal to Act 4) is not done and Duriel is.
		...
		Pather.moveTo(5148 + (me.gid & 1 ? 5 : -5), 5066 + (me.gid & 2 ? 5 : -5), 5);	//260813	//260822
