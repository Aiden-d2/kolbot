...
var ClassAttack = {
	...
	doCast: function (unit, timedSkill, untimedSkill) {
		var i, angle, fwd, side, tx, ty;	//260902
		...
		if (timedSkill > -1 && (!me.getState(121) || !Skill.isTimed(timedSkill))) {
			...
			case 245: // Tornado
				...
				fwd = rand(2, 3);
				side = rand(-1, 1);
				tx = Math.round(unit.x + Math.cos(angle) * fwd + Math.cos(angle + Math.PI / 2) * side);
				ty = Math.round(unit.y + Math.sin(angle) * fwd + Math.sin(angle + Math.PI / 2) * side);
				...
				if (!unit.dead) {
					Skill.cast(timedSkill, Skill.getHand(timedSkill), tx, ty);
				}
