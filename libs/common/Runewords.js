...
var Runeword = {
	...
	// gemmed
	FortitudeGem: me.ladder ? [610, 621, 623] : false, // El + Sol + Dol gemmed for Fortitude
	CallToGem: [620, 617, 632, 633], // Amn + Ral + Mal + Ist gemmed for CallToArms
...
RuneLoop:
	...
	checkItem: function (unit) { // for pickit
		...
		if (((unit.itemType >= 96 && unit.itemType <= 102) || unit.itemType === 74) && this.needList.indexOf(unit.classid) > -1) { // rune // & gem by eom
			return true;
		}
	...
	*/
	getBase: function (runeword, base, ethFlag, reroll) {
		...
		Gemmd1 = runeword == Runeword.CallToGem ? 1 : 0;	// CallToGem
		Gemmd2 = runeword == Runeword.FortitudeGem ? 1 : 0;	// FortitudeGem
		
		if (item) {
			do {
				if (item && (item.quality < 4 || ((item.classid === 198 || item.classid === 375) && item.quality === 7)) && item.getStat(194) === runeword.length + (Gemmd1 + Gemmd2)) { //eom
					...
					*/
					if ((!reroll && !item.getItem()) || (reroll && item.getItem() && !NTIP.Evaluate(item).result)) {	//260907
	...
	makeRunewords: function () {
		...
		while (true) {
			...
			//260901
			parts = items[0].fname.replace(/ÿc[0-9!"+<;.*]/g, "").split("\n").reverse();
			logString = parts[0].trim() + " " + parts.slice(1).join(" ").trim() + (items[0].getFlag(0x400000) ? " (Eth)" : "");	//260901
			me.overhead("[Runeword] " + logString);
			...
			if (result.result) {	//260814
				...
				Misc.itemLogger("Runeworded Fail", items[0]);
			}
		}
	...
	rerollRunewords: function () {
		var i, base, scroll, hel, parts, logString;	//260831
		...
		for (i = 0; i < Config.Runewords.length; i += 1) {
			...
			if (base) {
				...
				//260901
				parts = base.fname.replace(/ÿc[0-9!"+<;.*]/g, "").split("\n").reverse();
				logString = parts[0].trim() + " " + parts.slice(1).join(" ").trim() + (base.getFlag(0x400000) ? " (Eth)" : "");
				me.overhead("[Reset] " + logString);
