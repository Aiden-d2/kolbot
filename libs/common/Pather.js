...
var NodeAction = {
	...
	go: function (arg) {
		...
		/* if (me.area !== 39 && arg.prevNode && getDistance(arg.prevNode, arg.node) < 15) {	//260903
			return true;
		} */
	...
	killMonsters: function (arg) {
		if (arg.clearPath !== false) {
			if (me.area === 39 && !Attack.clear(25, typeof arg.clearPath === "number" ? arg.clearPath : 0)) {	//260901
	...
	useWells: function (arg) {	//260611
		if (!Config.UseWells || arg.clearPath === false) {
			return;
		}

		var well, well2, wellList = [],
			wellRange = Pather.useTeleport() ? 25 : 15,	//260901
			wellIDs = [111, 113, 115, 118, 130, 132, 137, 138, 322, 426, 493, 498, 513, 519];

		well = getUnit(2);

		if (well) {
			do {
				if (well.mode === 0 && wellIDs.indexOf(well.classid) > -1 && getDistance(me, well) <= wellRange && !checkCollision(me, well, 0x4)) {
					wellList.push(copyUnit(well));
				}
			} while (well.getNext());
		}
		
		if (wellList.length > 0) {
			wellList.sort(Sort.units);
	...
	moveTo: function (x, y, retry, clearPath, pop) {
		...
		function safeCheckNode(targetNode, monList, fireList, excludedNodes) {	//260807	//260823 order change
			...
			for (dist = Pather.teleDistance; dist >= Config.SafeTele.Min; dist -= Config.SafeTele.Step) {
				...
				for (i = 0; ; i += 1) {	//260524
					offset = i === 0 ? 0 : (i % 2 === 1 ? Math.ceil(i / 2) : -Math.ceil(i / 2)) * step;
					if (Math.abs(offset) > Config.SafeTele.Angle) {
						break;
					}
		...
		while (path.length > 0) {
			...
			if (!me.inTown && Config.TownCheck) {	//260516
				Misc.townCheck();
			}
			...
			//eom 260411
			if (useTeleport && !me.inTown && path.length > 0 && Config.SafeTele.Skip.indexOf(me.area) === -1) {	//260829
				if (!excludedNodes) {	// 260523
					excludedNodes = [];
				}
				
				monList = Attack.buildMonsterList();
				fireList = Attack.getFireList();	//260807
				
				checkedNode = safeCheckNode(node, monList, fireList, excludedNodes);	// 260829
				
				if (checkedNode !== node) {
					excludedNodes.push({x: checkedNode.x, y: checkedNode.y});	// 260823
					
					if (excludedNodes.length > 5) {	// 260823
						excludedNodes.shift();
					}
				...
				if (useTeleport ? this.teleportTo(node.x, node.y) : this.walkTo(node.x, node.y, (fail > 0 || me.inTown) ? 2 : 4)) {
					if (!me.inTown) {
						if (this.recursion) {
							...
							try {
								var nodeActionResult = NodeAction.go({clearPath: clearPath, prevNode: prevNode, node: node});	//260806
								
								if (nodeActionResult === "killMonsters") {
									return nodeActionResult;
								}
								
								if (!prevNode || getDistance(prevNode, node) >= 20) {	//260829
									prevNode = node;
								}
								
								while (!useTeleport && path.length > 1 && getDistance(me, path[0]) <= Pather.walkDistance) {
									path.shift();
								}

								if (useTeleport && getDistance(me, node.x, node.y) > 5) {
					...
					me.overhead("move retry " + fail);	//eom
...
ModeLoop:
		...
		for (i = 0; i < areas.length; i += 1) {
			...
			for (j = 0; j < exits.length; j += 1) {
				...
				if (currExit.target === areas[i]) {
					...
					if (!this.moveTo(dest[0], dest[1], 30, clearPath)) {
						return false;
					}
	...
	*/
	useWaypoint: function useWaypoint(targetArea) {
		switch (targetArea) {
		case undefined:
...
MainLoop:
	...
	*/
	goWP: function (area, clearPath) {
		var i, j, wp, presetUnit, attempt,	//eom 260411
	...
	journeyTo: function (area) {
		...
		while (target.course.length) {
			...
			if (me.inTown && this.wpAreas.indexOf(target.course[0]) > -1 && getWaypoint(this.wpAreas.indexOf(target.course[0]))) {
				this.useWaypoint(target.course[0]);
				Precast.doPrecast(false);
