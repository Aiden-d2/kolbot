/**
*	@filename	Merc.js
*	@desc		Packet-based mercenary aura prediction and selective hiring.
*			Reads the mercenary list packet (0x4e/0x4f) sent when the hire menu opens,
*			reproduces the client-side RNG to predict each candidate's aura before hiring,
*			then hires only the candidate matching the requested aura instead of hiring
*			blindly and checking afterward.
*
*			Ownership check uses DataFile (same convention as the original Town.hireMerc)
*			instead of querying the live unit, since me.getMerc() can be unreliable
*			immediately after joining a game.
*
*			Post-hire verification still queries the live unit via getSkill(); if the
*			prediction turns out wrong, the next matching candidate from the same packet
*			batch is retried (up to 5 times) without re-opening the NPC menu.
*/
...
var Merc = {
	...
	// hireable skill -> act and skills.txt id. skill names are unique across acts,
	// so the act is derived from the skill instead of being passed in, which removes any
	// MercType/MercSkill mismatch. id is used only for post-hire getSkill() verification.
	mercSkill: {	//260827
		"Fire Arrow":    {act: 1, id: 7, diff: null},
		"Cold Arrow":    {act: 1, id: 11, diff: null},
		"Prayer":        {act: 2, id: 99, diff: [0, 2]},
		"Defiance":      {act: 2, id: 104, diff: [0, 2]},
		"Blessed Aim":   {act: 2, id: 108, diff: [0, 2]},
		"Thorns":        {act: 2, id: 103, diff: [1]},
		"Holy Freeze":   {act: 2, id: 114, diff: [1]},
		"Might":         {act: 2, id: 98, diff: [1]},
		"Fire Ball":     {act: 3, id: 47, diff: null},
		"Lightning":     {act: 3, id: 49, diff: null},
		"Glacial Spike": {act: 3, id: 55, diff: null},
		"Bash":          {act: 5, id: 126, diff: null}
	},

	// Extracts the act indicator from the mercenary's locale name ID (a numeric packet field,
	// not a display string). Ranges correspond to which act's hireling table the name ID belongs to.
	checkActByName: function (name) {
		if (name >= 3411 && name <= 3451) return 1;
		else if (name >= 1019 && name <= 1039) return 2;
		else if (name >= 1040 && name <= 1059) return 3;
		else if (name >= 10835 && name <= 10901) return 5;
		else return false;
	},

	// 32x32-bit multiplication split into four 16-bit partial products to stay within
	// JS Number precision (53-bit safe integer limit). Used to reproduce the client-side
	// LCG step without an arbitrary-precision BigInt library.
	mul32x32: function (a, b) {
		...
		var mid = hiLo + loHi;
		var midCarry = mid >= 0x100000000 ? 1 : 0;

		var lo = (loLo + ((mid & 0xFFFF) * 0x10000)) >>> 0;
		var loCarry = (loLo + ((mid & 0xFFFF) * 0x10000)) >= 0x100000000 ? 1 : 0;

		var hi = (hiHi + Math.floor(mid / 0x10000) + midCarry * 0x10000 + loCarry) >>> 0;

		return { hi: hi >>> 0, lo: lo >>> 0 };
	},

	// Reproduces the D2 mercenary RNG (LCG). Verified equivalent to the original
	// arbitrary-precision (BigInt library based) implementation across 5000+ randomized
	// seeds and chained-call sequences, and independently cross-checked against a native
	// BigInt reference implementation.
	Rnd: function (seed) {
		this.hi = 666 >>> 0;
		this.lo = seed >>> 0;
		...
		this.roll = function () {
			var tmp = this.lo;
			var prod = Merc.mul32x32(tmp, 1791398085);

			var sumLo = prod.lo + this.hi;
			var carry = sumLo > 0xFFFFFFFF ? 1 : 0;
			sumLo = sumLo >>> 0;

			var sumHi = (prod.hi + carry) >>> 0;
	...
	// Represents a single mercenary candidate predicted from a packet's name/seed pair,
	// before any hire attempt is made. Mirrors the fields the original hire flow relied on.
	candidate: function (name, seed) {
		this.id = name;
		this.name = getLocaleString(name);

		var getTable = function () {
			var tablelist = [], level = 0;
			var act = Merc.checkActByName(name);
			for (var i = 0; i < Merc.table.length; i++) {
				if (Merc.table[i][5] == act && Merc.table[i][6] == me.diff + 1 && Merc.table[i][2] == me.gametype * 100) {
					if (level == 0 || Merc.table[i][7] == level) {
						tablelist.push(Merc.table[i]);
						level = Merc.table[i][7];
					}
				}
			}
			return tablelist;
		...
		var getMercInfo = function (index, infoindex) {
			if (validMercTable[index].hasOwnProperty(infoindex)) return validMercTable[index][infoindex];
			return false;
		};

		var validMercTable = getTable();
		if (!validMercTable.length) {	//260815
			this.skills = [];
			this.level = -1;

			return;
		}
		var newseed = new Merc.Rnd(seed);
		newseed.roll();
		...
		if (getMercInfo(index, 33)) this.skills.push({ name: getMercInfo(index, 33), lvl: Math.floor(lvl * 10 / 32) });
		if (getMercInfo(index, 39)) this.skills.push({ name: getMercInfo(index, 39), lvl: Math.floor(lvl * 10 / 32) });
		if (getMercInfo(index, 45)) this.skills.push({ name: getMercInfo(index, 45), lvl: Math.floor(lvl * 10 / 32) });

		this.cost = getMercInfo(index, 11) * (15 * difference + 100) / 100;
		if (this.cost < getMercInfo(index, 11)) this.cost = getMercInfo(index, 11);
		...
		if (this.dexterity < 10) this.dexterity = 10;
	...
	// Listens for the mercenary list packets sent when the hire NPC menu opens.
	// 0x4f clears the candidate list (new menu session), 0x4e adds one candidate
	// (name/seed pair) which is immediately resolved into a candidate prediction.
	...
	// Ownership check via DataFile instead of querying the live unit (me.getMerc()),
	// which was found to be unreliable immediately after joining a game (intermittent
	// false negatives leading to re-hiring an already-owned mercenary).
	// The !me.getMerc() guard still confirms a mercenary unit actually exists, so a
	// stale DataFile record (e.g. mercenary died and was never replaced) does not
	// cause a false "already have it" result.
	checkSkill: function (skill) {	//260815
		...
		if (!Misc.poll(function () { return me.getMerc(); }, 2000, 100)) {
	...
	// Opens the hire NPC menu, collects mercenary candidates via packet prediction,
	// hires the first candidate matching the requested skill/level/cost, then verifies
	// the actual result via getSkill(). If verification fails (prediction was wrong for
	// that candidate), the next matching candidate from the same packet batch is retried
	// without re-opening the menu, up to 5 attempts.
	//
	// skill: skill name string required. the act is derived from it via Merc.mercSkill;
	//        an unknown name or a difficulty-restricted name skips hiring and returns true.
	// force: if true, hires again even if checkSkill() says the skill is already owned.
	// level: minimum mercenary level required to consider a candidate. Defaults to 2.	//260827
	hire: function (skill, force, level) {
		if (!skill) {
			print("[hire] no skill specified");
			return true;
		}
		
		var check = Merc.mercSkill[skill];

		if (!check) {
			print("[hire] invalid skill");
			return true;
		}
		...
		if (this.checkSkill(skill) && !force) {
			return true;
		}

		if (check.diff && check.diff.indexOf(me.diff) < 0) {
			print("[hire] invalid difficulty");
			return true;
		}

		var act = check.act;
		
		if (act === 2 && !me.getQuest(7, 0)) {
			return false;
		}

		if (act === 3 && !me.getQuest(15, 0)) {
			return false;
		}

		if (act === 5 && !me.getQuest(28, 0)) {
			return false;
		}
		...
		var candidates = [];
		for (let merc of Merc.list) {
			for (let mercSkill of merc.skills) {
				if (mercSkill.name == skill && merc.level >= level && merc.cost <= me.gold) {
		...
		if (candidates.length > 0) {	//260828
			...
			print("[hire] no candidates: " + skill);
			failed = true;
		}
		...
		while (retry < 5 && candidates.length > 0) {
			var target = candidates.shift();

			target.hire();

			delay(me.ping * 2 + 500);

			var hiredMerc = me.getMerc();
			var skillId = check.id;

			if (hiredMerc && hiredMerc.getSkill(skillId, 1)) {
				foundMerc = true;
				D2Bot.printToConsole("[hire] hired " + skill + " | level : " + target.level, 5);
