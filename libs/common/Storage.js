...
var Container = function (name, width, height, location) {
	...
	this.Reset = function () {
		...
		this.itemList = [];
		this.openPositions = this.height * this.width;	//260816
...
Loop:
	...
	/* Container.MoveToSlot(item, col, row)
	 *	Places the item at an exact grid position, relocating blockers first.
	 */
	this.MoveToSlot = function (item, col, row) {	//260816
		var i, c, r, nx, ny, placed, overlap, tick, blocker, blockers, dup, target;

		if (!item) {
			return false;
		}

		if (col < 0 || row < 0 || col + item.sizex > this.width || row + item.sizey > this.height) {
			return false;
		}
		...
		for (r = row; r < row + item.sizey; r += 1) {
			for (c = col; c < col + item.sizex; c += 1) {
				if (this.buffer[r][c] > 0) {
					blocker = this.itemList[this.buffer[r][c] - 1];

					if (blocker && blocker.gid !== item.gid) {	// && blocker.classid !== item.classid) {	//260816
						dup = false;

						for (i = 0; i < blockers.length; i += 1) {
							if (blockers[i].gid === blocker.gid) {
								dup = true;
		...
		for (i = 0; i < blockers.length; i += 1) {
			blocker = getUnit(4, -1, -1, blockers[i].gid);
			...
			Spot:
			for (c = 0; c <= this.width - blocker.sizex; c += 1) {
				for (r = 0; r <= this.height - blocker.sizey; r += 1) {
					overlap = !(c + blocker.sizex <= col || c >= col + item.sizex ||
								r + blocker.sizey <= row || r >= row + item.sizey);
					...
					for (nx = 0; nx < blocker.sizex; nx += 1) {
						for (ny = 0; ny < blocker.sizey; ny += 1) {
							if (this.buffer[r + ny][c + nx] > 0) {
								placed = false;
					...
					if (placed) {
						if (blocker.toCursor()) {
							clickItem(0, c, r, this.location);

							tick = getTickCount();

							while (me.itemoncursor && getTickCount() - tick < Math.max(1000, me.ping * 3 + 500)) {
								delay(10);
							}
		...
		if (target.toCursor()) {
			...
			while (me.itemoncursor && getTickCount() - tick < Math.max(1500, me.ping * 3 + 1000)) {
				delay(10);
			}
		...
		target = getUnit(4, -1, -1, item.gid);	//260816
