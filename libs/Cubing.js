...
//260831
var RecipeName = [];
	RecipeName[1] = RecipeName[2] = RecipeName[3] = RecipeName[4] = RecipeName[5] = RecipeName[6] = RecipeName[7] = RecipeName[8] = RecipeName[9] = "[Craft] HitPower";
	RecipeName[10] = RecipeName[11] = RecipeName[12] = RecipeName[13] = RecipeName[14] = RecipeName[15] = RecipeName[16] = RecipeName[17] = RecipeName[18] = "[Craft] Blood";
	RecipeName[19] = RecipeName[20] = RecipeName[21] = RecipeName[22] = RecipeName[23] = RecipeName[24] = RecipeName[25] = RecipeName[26] = RecipeName[27] = "[Craft] Caster";
	RecipeName[28] = RecipeName[29] = RecipeName[30] = RecipeName[31] = RecipeName[32] = RecipeName[33] = RecipeName[34] = RecipeName[35] = RecipeName[36] = "[Craft] Safety";
	RecipeName[37] = RecipeName[38] = RecipeName[39] = RecipeName[40] = "[Upgrade] Unique";
	RecipeName[41] = RecipeName[42] = RecipeName[43] = RecipeName[44] = "[Upgrade] Rare";
	RecipeName[45] = RecipeName[46] = RecipeName[47] = RecipeName[48] = "[Socket]";
	RecipeName[49] = "[Reroll] Magic";
	RecipeName[50] = "[Reroll] Rare";
	RecipeName[51] = "[Reroll] HighRare";
	RecipeName[52] = "[Rune]";
	RecipeName[53] = "[Token Of Absolution]";
	RecipeName[54] = RecipeName[55] = "[Upgrade] White";
...
var Cubing = {
	...
	getCube: function () {
		...
		if (getScript(true).name === "threads\\townchicken.js") {
			return false;
		}
	...
	buildRecipes: function () {
		...
		for (i = 0; i < Config.Recipes.length; i += 1) {
			...
			case Recipe.Gem:
				...
				break;
			case Recipe.Moser2:
				this.recipes.push({Ingredients: [586, 586, 658], Index: Recipe.Moser2, AlwaysEnabled: true}); //eom
...
IngredientLoop:
			for (j = 0; j < this.recipes[i].Ingredients.length; j += 1) {
				for (k = 0; k < items.length; k += 1) {
					if (((this.recipes[i].Ingredients[j] === "pgem" && this.gemList.indexOf(items[k].classid) > -1) ||
						...
						this.validIngredients.push({classid: items[k].classid, gid: items[k].gid, recipe: this.recipes[i]});	//eom
	...
	checkItem: function (unit, result) { //260805
		if (!Config.Cubing) {
			return false;
		}
	...
	validItem: function (unit, recipe, result) {	//260805
		if (result === undefined) {	//260805
			result = NTIP.Evaluate(unit).result;
		}
		
		// Don't use items in locked inventory space
		if (unit.mode === 1 && unit.location === 3 && Storage.Inventory.IsLocked(unit, Config.Inventory)) {	//260516
			return false;
		}
		...
		if (recipe.Index >= Recipe.HitPower.Helm && recipe.Index <= Recipe.Safety.Weapon) {
			...
			if (unit.itemType === 58) {
				if (recipe.Enabled && result === 0) {	//260805
					return true;
				}
			// Main item, NOT matching a pickit entry
			} else if (unit.quality === 4 && Math.floor(me.charlvl / 2) + Math.floor(unit.ilvl / 2) >= recipe.Level && result === 0) {	//260805
				return true;
			}
		...
		if (recipe.Index >= Recipe.Unique.Weapon.ToExceptional && recipe.Index <= Recipe.Unique.Armor.ToElite) {
			...
			if (unit.quality === 7 && result === 1) {	//260805
				switch (recipe.Ethereal) {
				case 0:
				case undefined:
					return result === 1;	//260805
				case 1:
					return unit.getFlag(0x400000) && result === 1;	//260805
				case 2:
					return !unit.getFlag(0x400000) && result === 1;	//260805
				}
			}
		...
		if (recipe.Index >= Recipe.Rare.Weapon.ToExceptional && recipe.Index <= Recipe.Rare.Armor.ToElite) {
			...
			if (unit.quality === 6 && result === 1) {	//260805
				switch (recipe.Ethereal) {
				case 0:
				case undefined:
					return result === 1;	//260805
		...
		if (recipe.Index === Recipe.Reroll.Magic) {
			if (unit.quality === 4 && unit.ilvl >= recipe.Level && result === 0) {	//260805
				return true;
			}
		...
		if (recipe.Index === Recipe.Reroll.Rare) {
			if (unit.quality === 6 && result === 0) {	//260805
				return true;
			}
		...
		if (recipe.Index === Recipe.Reroll.HighRare) {
			if (recipe.Ingredients[0] === unit.classid && unit.quality === 6 && result === 0) {	//260805
				recipe.Enabled = true;
		...
		if (recipe.Index === Recipe.LowToNorm.Armor || recipe.Index === Recipe.LowToNorm.Weapon) {
			if (unit.quality === 1 && result === 0) {	//260805
				return true;
			}
			...
			if (items) {
				...
				if (items) {
					for (j = 0; j < items.length; j += 1) {
						...
						case 0:
							Misc.itemLogger("Transmuted Fail", items[j]);
							items[j].drop();
	...
	emptyCube: function () {
		...
		while (items.length) {
			if (!Storage.Inventory.MoveTo(items[0])) { // && !Storage.Stash.MoveTo(items[0])) { //eom
				return false;
			}
