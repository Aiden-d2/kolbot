...
MainLoop:
	...
	isTimed: function (skillId) {
		return [15, 25, 27, 51, 56, 59, 62, 64, 121, 154, 225, 223, 228, 229, 234, 244, 247, 249, 250, 256, 268, 275, 277, 279].indexOf(skillId) > -1;
	},
...
var Equip = {
	updateTiers: function () {  //260804
		var goal = true;

		for (var i = 1; i <= 10; i += 1) {
			this.updateTier(i);

			if (Config.TierNow[Config.Gear[i]].tier < Config.TierGoal[Config.Gear[i]]) {
				Config.TieredGear[Config.Gear[i]] = false;	//260815
				goal = false;
			} else {
				Config.TieredGear[Config.Gear[i]] = true;	//260805
			}
		}
	...
	updateTier: function (bodyLoc, skipLog) {    //260804
		var key = Config.Gear[bodyLoc];
		
		if (!key) {
			return false;
		}
		
		var eq = this.getEquippedItem(bodyLoc);
		
		Config.TierNow[key] = {tier: eq.tier, str: eq.str, dex: eq.dex};
		
		if (!skipLog) {
			print("equipped: [" + key + "] " + eq.tier);
		}
	...
	canEquip: function (item, bodyLoc) {
		...
		let curr = this.getEquippedItem(bodyLoc);
		
		if (item.getStat(92) > me.getStat(12) || item.strreq > me.getStat(0) - curr.str || item.dexreq > me.getStat(2) - curr.dex) { // Higher requirements
			return false;
		}
	...
	getBodyLoc: function (item) {
		...
		case 25: // wand
			if (me.classid === 4) {
				if (item.classid === 103 && item.quality === 7) {
					bodyLoc = 5;
				} else {
					bodyLoc = [4, 5];
				}
		...
		case 28: // axe
			if (me.classid === 4) {
				...
				case 102: // ancientaxe
				case 201: // feralaxe
				case 202: // silveredgedaxe
				case 203: // decapitator
				case 204: // championaxe
				case 205: // gloriousaxe
					bodyLoc = 5;
		...
		case 85: // amazonbow
		case 86: // amazonspear
		case 87: // amazonjavelin
			bodyLoc = 4;

			break;
		case 88: // assassinclaw
			/*switch (item.classid) {	//eom *need to validate
				case 175: // 
				case 176: // 
				case 177: // 
				case 178: // 
	...
	autoEquip: function () {
		...
		function sortEq(a, b) {
			return NTIP.GetScore(b, "Tier") - NTIP.GetScore(a, "Tier");
		}
		...
		for (i = 0; i < items.length; i += 1) {
			if (NTIP.GetScore(items[i], "Tier") === 0) {
				items.splice(i, 1);
		...
		while (items.length > 0) {
			...
			tier = NTIP.GetScore(items[0], "Tier");
			...
			if (tier > 0 && tier < 100 && bodyLoc) {
				for (j = 0; j < bodyLoc.length; j += 1) {
					...
					var equipped = this.getEquippedItem(bodyLoc[j]);
					
					if ([3, 7].indexOf(items[0].location) > -1 && tier > equipped.tier && equipped.classid !== 174) { // khalim's will adjustment
						...
						if (this.equip(items[0], bodyLoc[j])) {
							var result = NTIP.Evaluate(items[0]);	//260805
							
							me.overhead("[Tier " + tier + "] " + items[0].name);
							Misc.logItem("Equipped [" + tier + "]", items[0], result.line);	//eom
...
var Grant = {
	updateTiers: function () {
		...
		this.updateStat(merc);
		
		var slots = [1, 3, 4];
		
		for (var i = 0; i < slots.length; i += 1) {
			this.updateTier(slots[i]);
		}
	...
	updateTier: function (bodyLoc, skipLog) {    //260804
		...
		Config.TierMerc[key] = {merc: eq.tier, str: eq.str, dex: eq.dex};
		
		if (!skipLog) {
			print("granted: [" + key + "] " + eq.tier);
		}
	...
	canEquip: function (item, bodyLoc, cache) {
		if (item.type !== 4) { // Not an item
			return false;
		}
		...
		if (cache) {
			if (Config.MercStat.lvl === 0) {	// no cached info
				return false;
			}
			
			var merc = Config.MercStat;
		} else {
			var merc = me.getMerc();
		...
		if (cache) {
			var curr = Config.TierMerc[Config.Gear[bodyLoc]],
				lvl = merc.lvl,
				str = merc.str,
				dex = merc.dex;
		} else {
		...
		if (item.getStat(92) > lvl || item.strreq > str - curr.str || item.dexreq > dex - curr.dex) { // Higher requirements
			return false;
		}
	...
	equip: function (item, bodyLoc, tier) {	//260805
		if (!this.canEquip(item, bodyLoc)) {
			return false;
		}
		
		var i, cursorItem, target;	//260815
		...
		for (i = 0; i < 3; i += 1) {
			...
			if (item.toCursor()) {
				...
				if (merc) {
					var mercItem = merc.getItem();

					if (mercItem) {
						do {
							if (mercItem.bodylocation === bodyLoc && mercItem.location === 1) {
								target = mercItem;	//260815
							}
						} while (mercItem.getNext());
					}
				}
	...
	autoEquip: function () {
		if (!Config.AutoEquip || !me.getMerc()) {	//260821
			return false;
		}
		...
		function sortEq(a, b) {
			return NTIP.GetScore(b, "Merc") - NTIP.GetScore(a, "Merc");
		}
		...
		// Remove items without tier
		for (i = 0; i < items.length; i += 1) {
			if (NTIP.GetScore(items[i], "Merc") === 0) {
				items.splice(i, 1);
		...
		while (items.length > 0) {
			...
			tier = NTIP.GetScore(items[0], "Merc");
			bodyLoc = this.getBodyLoc(items[0]);
			...
			if (tier > 0 && tier < 100 && bodyLoc) {
				for (j = 0; j < bodyLoc.length; j += 1) {
					...
					if ([3, 7].indexOf(items[0].location) > -1 && tier > equipped.tier) {
						if (!items[0].getFlag(0x10)) { // unid
							tome = me.findItem(519, 0, 3);
							scroll = me.findItem(530, 0, 3);
						...
						var grant = this.equip(items[0], bodyLoc[j], tier);	//260805
	...
	removeItems: function () {
		...
		if (items) {
			for (var i = 0; i < items.length; i++) {
				clickItem(4, items[i].bodylocation);	//260829 memo: clickItem(4) = merc, not 3
				delay(me.ping * 2 + 500);
...
var Misc = {
	...
	itemLogger: function (action, unit, text) {
		...
		return this.fileAction("_cache/ItemLog.txt", 2, dateString + " <" + me.profile + "> <" + action + "> <" + lastArea + "> (" + Pickit.itemQualityToName(unit.quality) + ") " + desc + (text ? " {" + text + "}" : "") + "\n");
	},
	...
	logItem: function (action, unit, keptLine) {
		...
		if (!Config.LogEssences && ["tes", "ceh", "bet", "fed"].indexOf(unit.code) > -1) {
			return false;
		}
		...
		if (!Config.LogMiddleRunes && ["r15", "r16", "r17", "r18", "r19", "r20", "r21"].indexOf(unit.code) > -1) {
			return false;
		}

		if (!Config.LogHighRunes && ["r22", "r23", "r24", "r25", "r26", "r27", "r28", "r29", "r30", "r31", "r32", "r33"].indexOf(unit.code) > -1) {
			return false;
		}
	...
	// Log someone's gear
	spy: function (name) {
		if (!isIncluded("tools/OOG.js")) {
			include("tools/OOG.js");
		}

		if (!isIncluded("libs/Prototypes.js")) {
...
var Experience = {
	...
	gainPercent: function () {
		return me.getStat(12) === 99 ? 0 : (this.gain() * 100 / this.nextExp[me.getStat(12)]).toFixed(2);
	},
