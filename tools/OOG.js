...
var DataFile = {
	...
	getObj: function () {
		...
		if (!FileTools.exists("_cache/" + me.profile + ".json")) {
			DataFile.create();
		}
...
MainLoop:
	...
	createGame: function (name, pass, diff, delay) {
		...
		if (delay) {
			this.timeoutDelay("Make Delay", delay);
		}
	...
	makeAccount: function (info) {
		...
		while (getLocation() !== 42) {// cycle until in empty char screen
			...
			case 34: // e-mail
				...
				break;
			case 16: // please wait	// 260907
			case 21: // connecting	// 260907
				break;
			default:
				print("makeAccount: location " + getLocation());
				break;
			}
